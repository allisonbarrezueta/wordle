import { evaluateGuess, getRandomWord, isValidWord, LetterState } from "../word.utils";
...
describe("word-utils", () => {
    ...
    describe("evaluateGuess", () => {
        ...
        it("returns some matches, misses and exists", () => {
            ...
            expect(result).toEqual([LetterState.MATCH, LetterState.MISS, LetterState.MISS, LetterState.MISS, LetterState.PRESENT]);
        });
        ...
        it("when 2 letters are present but answer has only 1 of those letters", () => {
            ...
            expect(result).toEqual([LetterState.MISS, LetterState.MISS, LetterState.PRESENT, LetterState.MISS, LetterState.MISS]);
            result = evaluateGuess("grook", "knock");
            expect(result).toEqual([LetterState.MISS, LetterState.MISS, LetterState.MATCH, LetterState.MISS, LetterState.MATCH]);
        });
        it("when 1 letter matches but guess has more of the same letter", () => {
            const result = evaluateGuess("colon", "allol");
            expect(result).toEqual([LetterState.MISS, LetterState.MISS, LetterState.MATCH, LetterState.MATCH, LetterState.MISS]);
        });
    });
